...
class OpenAIProvider(LLMProvider):
    ...
    def __init__(self, settings: Optional[Settings] = None):
        ...
        self.openai_settings = self.settings.openai
        
        # Initialize the pydantic-ai OpenAI provider if available
        self.pydantic_provider = None
        if PyAIOpenAIProvider is not None:
            try:
                self.pydantic_provider = PyAIOpenAIProvider(
                    api_key=self.openai_settings.api_key.get_secret_value(),
                    organization=self.openai_settings.organization_id,
        ...
        self.client = httpx.AsyncClient(
            base_url=self.openai_settings.base_url,
            headers={
                "Authorization": f"Bearer {self.openai_settings.api_key.get_secret_value()}",
                "Content-Type": "application/json"
            }
        )
        if self.openai_settings.organization_id:
            self.client.headers["OpenAI-Organization"] = self.openai_settings.organization_id
    ...
    async def generate_completion(self, prompt: str, **kwargs) -> Dict[str, Any]:
        ...
        """
        try:
            # Convert to chat completion since OpenAI deprecated completions endpoint
    ...
    async def generate_chat_completion(
        ...
        **kwargs
        ...
        """
        try:
            model = kwargs.pop("model", self.openai_settings.completion_model)
